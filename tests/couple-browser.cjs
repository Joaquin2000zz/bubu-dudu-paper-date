...
// CI renders with software WebGL; wall time can advance faster than simulation time.
await page.keyboard.press('j');await page.waitForFunction(()=>window.__BUBU_DUDU_PAPER_DATE__.positions.Dudu.jumpY>.1);await page.waitForFunction(()=>window.__BUBU_DUDU_PAPER_DATE__.positions.Dudu.jumpY===0);
for(let i=0;i<5;i++)await page.click('#cameraRight');await page.waitForTimeout(600);
await page.evaluate(()=>{const a=window.__BUBU_DUDU_PAPER_DATE__;a.debugNear();a.giveFlowers()});await page.waitForFunction(()=>{const s=window.__BUBU_DUDU_PAPER_DATE__.state;return s.mode==='flowers'&&s.eventT>1.4});
...
await page.waitForFunction(()=>window.__BUBU_DUDU_PAPER_DATE__.state.mode==='ready');await page.evaluate(()=>window.__BUBU_DUDU_PAPER_DATE__.kiss());await page.waitForFunction(()=>{const s=window.__BUBU_DUDU_PAPER_DATE__.state;return s.mode==='kiss'&&s.eventT>.6});await check();await page.screenshot({path:'tests/artifacts/kiss-profile.png'});
...
await page.waitForFunction(()=>window.__BUBU_DUDU_PAPER_DATE__.state.mode==='done');assert.equal(await page.evaluate(()=>window.__BUBU_DUDU_PAPER_DATE__.state.kisses),1);assert.deepEqual(errors,[]);console.log('PASS: final garden jump/landing, gift and kiss at rotated camera angles, face spacing, shared floor height, dialogue, and completed kiss.');
